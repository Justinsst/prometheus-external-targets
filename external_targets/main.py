...
def main():
    ...
    while True:
        ...
        for hostname in target_hostnames.splitlines():
            ...
            if not ip:
                logging.error(
                    f"DNS lookup failed for host {hostname}. "
...
def parse_args():
    ...
    parser.add_argument(
        "-k",
        "--kubeconfig",
        help="Path to the cluster's kubeconfig. If not specified, the \
              in-cluster config is used (source namespace is assumed to be on \
